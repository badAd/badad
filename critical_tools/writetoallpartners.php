...
// Access from webdir with: writetoallpartners.php?k=LONGWRITERSKEY
...
$canned_email = "vitalallpartnernotice"; // Slug from the "pantry" table to select the canned email
$subject_suffix = " - badAd Partner Update"; // Appends the canned email Subject
$payload_content = $messageBody; // Middle of the Body, after the canned email and before the salutation
$footer_link_content = "";

echo "<br />Sending message to all partners:<br />
time: $timeNow<br />
...
$qp = "SELECT user_id FROM partners WHERE email_confirmed='Confirmed'";
$rowsp = mysqli_query ($dbc, $qp);
while ($rowp = mysqli_fetch_array($rowsp, MYSQLI_NUM)) {
  // Set the email value
  $partnerID = "$rowp[0]";

  // Get user's info from the database
  $qea = "SELECT userid FROM emailwrongunsubscribe WHERE useable='live' AND userid='$partnerID'";
  $rowsea = mysqli_query ($dbc, $qea);
  while ($rowea = mysqli_fetch_array($rowsea, MYSQLI_NUM)) {
