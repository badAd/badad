...
$q = "SELECT date_dead, useable FROM confirmpartnerchange WHERE binary confirmkey='$URLconfirmkey'";
...
if (($timeNow < $datedead) && ($usable == 'live')) {
  // Disable the temporary link
	$qt = "UPDATE confirmpartnerchange SET useable='dead' WHERE binary confirmkey='$URLconfirmkey'";
	if ($rt = mysqli_query ($srv_dbc, $qt)) { // If it ran OK.

    // Cleanup any verify message
    if (isset($_SESSION['partner_must_verify'])) {unset($_SESSION['partner_must_verify']);}
