...
$q = "SELECT time_date, source, keytext, subkey, filter FROM seen_ad_analytics WHERE ad_id='$adID' AND CAST(time_date AS DATE) >= '$start_date' AND CAST(time_date AS DATE) <= '$end_date' ORDER BY time_date";
$r = mysqli_query ($srv_dbc, $q);
// Activity check
if (mysqli_num_rows($r) == 0) {
	echo "
...
// Loop through each entry in the date range and put them into an array
// Use CAST(time_date AS DATE) per https://dba.stackexchange.com/questions/108287/why-does-my-query-search-datetime-not-match
$q = "SELECT time_date, source, keytext, subkey, filter FROM clicked_ad_analytics WHERE ad_id='$adID' AND CAST(time_date AS DATE) >= '$start_date' AND CAST(time_date AS DATE) <= '$end_date' ORDER BY time_date";
$r = mysqli_query ($srv_dbc, $q);
// Activity check
if (mysqli_num_rows($r) == 0) {
	echo "
