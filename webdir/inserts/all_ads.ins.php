...
$q = "SELECT id, date_expires, category_id, subcat_id, role_id, tag_ids, ad_content_hdng, ad_content_dscr, ad_content_info, ad_content_pyrt, ad_content_cntc, ad_content_bizn, ad_biz_listing, week_view_count, epoch_wk_reset FROM ads WHERE $SQLdateCondition $and_filter ORDER BY week_view_count, epoch_wk_reset, id DESC LIMIT $itemskip,$pageitems";
$row = mysqli_query($dbc, $q);
	...
	while ($ad_item = mysqli_fetch_array($row)) {
			...
			if ($timeNowEpoch >= $ad_epoch_dead) {
				...
				$r = mysqli_query ($dbc, $q);
				//if (mysqli_affected_rows($dbc) != 1) {sql_error($q, 'dbc', "sqle_all_ads");}
				continue;
			}
      ...
      $role_slug = "$rlarow[2]";
			...
			// Prep the link
			$List_ad_content_cntc_rd = "https://$adServeDomain/v/$List_serialno/ct.html";
			// Render the ad
	    ...
	    $q = "UPDATE ads SET week_view_count='$newWeekCount' WHERE id='$ad_id'";
	    $r = mysqli_query ($dbc, $q);
	    //if (mysqli_affected_rows($dbc) != 1) {sql_error($q, 'dbc', "sqle_all_ads");}

			// Ad count
	    $acq = "SELECT view_count FROM listads WHERE ad_id='$ad_id'";
	    ...
	    $newCount = $adcount +1;
	    // Update the new count
	    $q = "UPDATE listads SET view_count='$newCount' WHERE ad_id='$ad_id'";
	    $r = mysqli_query ($srv_dbc, $q);
			//if (mysqli_affected_rows($srv_dbc) != 1) {sql_error($q, 'srv_dbc', "sqle_all_ads");}

			// Update the analytics
			$aq = "INSERT INTO seen_ad_analytics (ad_id, source, filter, time_date, time_epoch) VALUES ('$ad_id', 'view', '$analytics_filter', '$timeNow', '$timeNowEpoch')";
	...
	if ((isset($_SESSION['filter_b'])) && ($_SESSION['filter_b'] == true)) {
		echo '<h4 class="ads">There are currently no BUSINESS ads to list.</h4><h4 class="ads">You might <a href="new_ad.php">run an ad</a> for what you need&mdash;buying, selling, or both.</h4>';
	} else {
		echo '<h4 class="ads">There are currently no ads to list.</h4><h4 class="ads">You might <a href="new_ad.php">run an ad</a> for what you need&mdash;buying, selling, or both.</h4>';
	}
}
