...
$itemskip = $pageitems * ($paged - 1);
	...
	$q = "SELECT id FROM partnersites WHERE directory_listed='listed' AND NOT type='podcast' AND NOT domain='podcast'";
...
// Message
echo '<p style="text-align: center;">These are blogs proudly sponsored by badAd and its advertisers.</p>';

// Search box
echo '<div id="searchListedWrap"><form action="blogs.php" method="get">
...
$q = "SELECT id, domain, directory_name, directory_url FROM partnersites WHERE directory_listed='listed' AND NOT type='podcast' AND NOT domain='podcast' $SQLcolumnSearch ORDER BY directory_name, domain DESC LIMIT $itemskip,$pageitems";
$row = mysqli_query($srv_dbc, $q);
	if (mysqli_num_rows($row) > 0) {
	while ($blog_item = mysqli_fetch_array($row)) {
			// Assign variables
			$blog_id = $blog_item[0];
			$domain = $blog_item[1];
			$directory_name = $blog_item[2];
			$directory_url = $blog_item[3];

			echo '<tr><td align="right"><big><a target = "_blank" title="'.$directory_url.'" href="'.$directory_url.'"><i>'.$directory_name.'</i></a></big></td><td align="left"><big><a target = "_blank" title="'.$directory_url.'" href="'.$directory_url.'">'.$domain.'</a></big></td></tr>';
	...
	echo '<h4 class="ads">There are currently no blogs for that search.</h4>';
} else {
	echo '<h4 class="ads">There are currently no blogs to list.</h4>';
}
