...
// See if user owns the podcast ad & get Date started
$adID = mysqli_real_escape_string ($dbc, $adID);
$q = "SELECT date_starts, ad_nickname, podcast_ad FROM ads WHERE id='$adID' AND user_id='$userid' AND ad_biz_listing='biz' AND NOT podcast_ad='0'";
...
$q = "SELECT ad_id FROM pod_ads WHERE ad_id='$adID' AND customer_user='$userid'";
$r = mysqli_query ($dbc, $q);
if (mysqli_num_rows($r) == 0) {
	header("Location: order_history.php");
	exit(); // Quit the script
...
// Set the date range & time settings
	...
	// Add empty date arrays
		if ((isset($_POST['cust_date_calpick'])) && ($_POST['cust_date_calpick'] == true)) { // Deal with a rounding fluke and date picking
			if ($chart_interval == 'day') {$num_download_entries = floor($date_difference / (60 * 60 * 24));}
			if ($chart_interval == 'hour') {$num_download_entries = floor($date_difference / (60 * 60));}
		} else {
			if ($chart_interval == 'day') {$num_download_entries = round($date_difference / (60 * 60 * 24));}
			if ($chart_interval == 'hour') {$num_download_entries = round($date_difference / (60 * 60));}
		}
...
// Loop through each entry in the date range and put them into an array (DOWNLOADS)
	// Use CAST(time_date AS DATE) per https://dba.stackexchange.com/questions/108287/why-does-my-query-search-datetime-not-match
	$q = "SELECT time_date, id FROM downloaded_ad_analytics WHERE ad_id='$adID' AND time_date >= '$start_date' AND time_date <= '$end_date' ORDER BY time_date";
			...
			echo "<h3>Nothing here!</h3><br />";
				...
				while ($adRow = mysqli_fetch_array($r)) {
				  ...
				  if ($chart_interval == 'day') {$time_mark_long = date_format($date_entry,"Y-m-d"); $entry_mark = date_format($date_entry,"d");}
					...
					// Very first entry
					if (!isset($last_time_download_mark)) {
						$last_time_download_mark = $time_mark_long;
						$downloadCount = 0;
						$largest_count = 0;
					}

					// Current time period
					if ($time_mark_long == $last_time_download_mark) {
						$downloadCount = $downloadCount + 1;

					// Up to a new time period this cycle
					} else {
						$last_time_download_mark = $time_mark_long; // New time mark
						$arrKey = $arrKey + 1; // Next array key entry
						$downloadCount = 1; // First download for the time mark
					}

					// Make the entry
					$entry = array($downloadCount, $time_mark_long, $entry_mark);
					if ($downloadCount > $largest_count) {$largest_count = $downloadCount;}
					$arrayHits[$arrKey] = $entry;

				} // Date range loop

// Each entry array (DOWNLOADS)

// Loop through each entry in the date range and put them into an array (CLICKS)
	// Use CAST(time_date AS DATE) per https://dba.stackexchange.com/questions/108287/why-does-my-query-search-datetime-not-match
	$q = "SELECT time_date, id FROM clicked_pod_ad_analytics WHERE ad_id='$adID' AND CAST(time_date AS DATE) >= '$start_date' AND CAST(time_date AS DATE) <= '$end_date' ORDER BY time_date";
	$r = mysqli_query ($srv_dbc, $q);
				...
				while ($adRow = mysqli_fetch_array($r)) {
				  ...
				  if ($chart_interval == 'day') {$time_mark_long = date_format($date_entry,"Y-m-d"); $entry_mark = date_format($date_entry,"d");}
					...
					// Up to a new time period this cycle
						...
						$arrKey = $arrKey + 1; // Next array key entry
						$clickCount = 1; // First download for the time mark
					}
...
// Each period array (DOWNLOADS)
				// Calculate each entry
				foreach ($arrayHits as $downloadCount) {
					$arrayHitPercents[] = $downloadCount[0] / $largest_count * 100;
				} // Percentage array
				...
				while ($stat_entry <= $num_download_entries+1) { // This needs to add 1 because of a calculation fluke about rounding and what a range actually is

					// Check & Set the entry
					if ((array_key_exists($arrsKey, $arrayHits)) && ($arrayHits[$arrsKey][1] == $current_period_long)) {
						$downloadCount = $arrayHits[$arrsKey][0];
						$downloadPercent = $arrayHitPercents[$arrsKey];
						// Done, increment
						$arrsKey = $arrsKey + 1;
					} else {
						$downloadCount = 0;
					...
					// Make the entry
					$nice_downloads = nice_number($downloadCount);
					$entry = array($current_period_long, $current_period_entry, $downloadCount, $downloadPercent, $nice_downloads);
...
//// End loop through each entry in the date range and put them into an array (DOWNLOADS)
...
//// End loop through each entry in the date range and put them into an array (CLICKS)
			...
			// Populate the chart
				...
				while ($stat_entry <= $num_entries) {
					echo '
					<div class="stat_entry" title="'.$arrayHitEntries[$stat_key][2].' downloads, '.$arrayClickEntries[$stat_key][0].' clicks, '.$arrayHitEntries[$stat_key][0].'">';
			...
			echo '<a href="'.$url_range_name.'_podcast_ad_stats.xml" download="badad_'.$file_range_name.'.xml">Download raw XML for this range ('.$start_date.' - '.$end_date.')</a>
			<br />
			<br />
			';
...
<div class="choose_stat_range">
	...
	echo '<div class="recent_stat_range_button" style="display:inline-block; margin:0.5em;">
		<form id="last48" action="podcast_ad_stats.php" method="post">
	...
	// Last 15 days
	echo '<div class="recent_stat_range_button" style="display:inline-block; margin:0.5em;">
		<form id="last15" action="podcast_ad_stats.php" method="post">
	...
	// Last 31 days
	echo '<div class="recent_stat_range_button" style="display:inline-block; margin:0.5em;">
		<form id="last31" action="podcast_ad_stats.php" method="post">
	...
	// Last 92 days
	echo '<div class="recent_stat_range_button" style="display:inline-block; margin:0.5em;">
		<form id="last92" action="podcast_ad_stats.php" method="post">
	...
	// Last 183 days
	echo '<div class="recent_stat_range_button" style="display:inline-block; margin:0.5em;">
		<form id="last183" action="podcast_ad_stats.php" method="post">
	...
	// Last 367 days
	echo '<div class="recent_stat_range_button" style="display:inline-block; margin:0.5em;">
		<form id="last376" action="podcast_ad_stats.php" method="post">
