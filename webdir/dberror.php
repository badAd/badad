...
if ((isset($_POST['sql_error'])) && ($_POST['sql_error'] == true) && (isset($_POST['sql_error_time']))) {

  // Validate the time
  $format = 'Y-m-d H:i:s';
  $dateTime = DateTime::createFromFormat($format, $_POST['sql_error_time']);
  if ($dateTime instanceof DateTime && $dateTime->format('Y-m-d H:i:s') == $_POST['sql_error_time']) {
    // Get the time
    $errorTimeNow = $_POST['sql_error_time'];
  } else {
    $IP = get_ip_addr(); script_kiddy('sk_89', '_POST sql_error_time', $_POST['sql_error_time'], $IP);
