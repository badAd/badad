...
// _POST the site ID
if ((isset($_POST['s'])) && (preg_match ('/[^a-zA-Z0-9]$/i', $_POST['s']))) {$IP = get_ip_addr(); script_kiddy('sk_67', '_POST s', $_POST['s'], $IP);}
...
$q = "UPDATE partnersites SET horizontal_inline=false WHERE id='$site_id'";
if ($r = mysqli_query ($srv_dbc, $q)) {
  ...
  <form id="psite_horizantal_form_'.$site_id.'">
    ...
    <button type="button" class="formbutton_orange" title="Switch on" onclick="ajaxFormData("psite_horizantal_form_'.$site_id.'", "partnersite_horizantal_on.ajax.php", "psite_status_td_'.$site_id.'");">vertical</button>
  </form>';
  exit(); // Quit the script

} else {
  sql_error($qr, 'srv_dbc', "sqle_14");
}
