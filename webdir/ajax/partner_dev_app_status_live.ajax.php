...
$q = "UPDATE devkeys SET status='live' WHERE id='$dev_app_id'";
if ($r = mysqli_query ($srv_dbc, $q)) {
  ...
  // This is incomplete and needs to contain the entire <div id="dev_key_mode_div_'.$dev_app_id.'">, including
    // $dev_old_pub_key, $dev_old_pub_key, $dev_live_pub_key
  echo '<br>
  <form id="dev_key_mode_form_'.$dev_app_id.'">
  	<input type="hidden" name="s" value="'.$dev_app_id.'">
  	<button type="button" class="formbutton_yellow" title="Switch to live mode" onclick="ajaxFormData("podcast_status_form_'.$dev_app_id.'", "partner_dev_app_status_live.ajax.php", "podcast_status_td_'.$site_id.'");">test</button>
  </form>';

  exit(); // Quit the script

} else {
  sql_error($q, 'srv_dbc', "sqle_97");
}
