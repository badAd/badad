...
// _POST the site ID
if ((isset($_POST['s'])) && (preg_match ('/[^a-zA-Z0-9]$/i', $_POST['s']))) {$IP = get_ip_addr(); script_kiddy('sk_57', '_POST s', $_POST['s'], $IP);}
...
$q = "UPDATE partnersites SET useable='live' WHERE id='$site_id'";
if ($r = mysqli_query ($srv_dbc, $q)) {
  ...
  $site_domain = "$row[0]";
  $_SESSION['on_domain'] = "$site_domain (ID #$site_id)";
