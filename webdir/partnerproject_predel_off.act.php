...
// _POST the site ID
if ((isset($_POST['s'])) && (preg_match ('/[^a-zA-Z0-9]$/i', $_POST['s']))) {$IP = get_ip_addr(); script_kiddy('sk_56', '_POST s', $_POST['s'], $IP);}
...
if ($r = mysqli_query ($srv_dbc, $q)) {
  ...
  $site_domain = "$row[0]";
  $_SESSION['off_domain'] = "$site_domain (ID #$site_id)";
