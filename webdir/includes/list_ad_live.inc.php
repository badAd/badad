...
// Update the status
$q = "UPDATE listads SET pub_status='live' WHERE ad_id='$List_ad_ID'";
$r = mysqli_query ($srv_dbc, $q);

if (!$r) { // If it didn't run okay
	sql_error($q, 'srv_dbc', "sqle_173");
}

// Update the status
$q = "UPDATE podcastads SET pub_status='live' WHERE ad_id='$List_ad_ID'";
$r = mysqli_query ($agg_dbc, $q);

if (!$r) { // If it didn't run okay
	sql_error($q, 'agg_dbc', "sqle_174");
}
